...
from debussy_concert.core.movement.protocols import PStartPhrase, PEndPhrase
...
class ReverseEtlMovement(MovementBase):
    ...
    def reverse_etl_bucket_uri_prefix(self):
        return (
            f"gs://{self.config.environment.reverse_etl_bucket}/"
    ...
    def datawarehouse_reverse_etl_extraction_query(self):
        return self.movement_parameters.extraction_query_from_temp.format(
            reverse_etl_table_uri=self.reverse_etl_table_uri
        )

    def setup(self, movement_parameters: ReverseEtlMovementParameters):
